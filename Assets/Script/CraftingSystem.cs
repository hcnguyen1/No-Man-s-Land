...
public class CraftingSystem
{
    ...
    // Check whether grid matches the shaped recipe exactly (all recipe slots match and other slots empty)
    public bool MatchesRecipe(CraftingRecipe recipe)
    {
        if (recipe == null) return false;

        // create a lookup for ingredients by position
        ...
        foreach (var ing in recipe.ingredients)
        {
            ...
            if (c.item != ing.item) return false; // compare by reference
    ...
    // Apply recipe: consumes ingredients from the grid. Returns true on success and reduces quantities accordingly.
